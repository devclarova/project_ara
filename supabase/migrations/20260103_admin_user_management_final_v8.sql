-- [FIX] Unified Admin Fixes V8: Sorting, Reports, Localization
-- 1. get_admin_users_list: Default to recent activity, correct NULLS LAST, consistent report count
-- 2. get_user_related_reports: Robust lookup for received reports
-- 3. get_user_made_reports: Robust lookup for sent reports

-- DROP EXISTING
DROP FUNCTION IF EXISTS public.get_admin_users_list(int, int, text, text, text, text, text, text, text, text, text);
DROP FUNCTION IF EXISTS public.get_user_related_reports(UUID);
...
CREATE OR REPLACE FUNCTION public.get_admin_users_list(
  ...
  sort_by text DEFAULT 'last_active_desc' -- Default to Recent Activity
)
RETURNS TABLE (
  id uuid,
  profile_id uuid,
    ...
    SELECT 
      ...
      (
        ...
        WHERE 
          ...
          OR
          (r.target_type = 'tweet' AND r.target_id IN (SELECT t.id FROM public.tweets t WHERE t.author_id = pp_inner.id))
          OR
          (r.target_type = 'reply' AND r.target_id IN (SELECT tr.id FROM public.tweet_replies tr WHERE tr.author_id = pp_inner.id))
          OR
...
BEGIN
    ...
    WHERE 
        ...
        OR
        (r.target_type = 'tweet' AND r.target_id IN (SELECT t.id FROM public.tweets t WHERE t.author_id = v_profile_id))
        OR
        (r.target_type = 'reply' AND r.target_id IN (SELECT tr.id FROM public.tweet_replies tr WHERE tr.author_id = v_profile_id))
        OR
        (r.target_type = 'chat' AND r.target_id IN (SELECT c.id FROM public.direct_chats c WHERE (c.user1_id = v_profile_id OR c.user2_id = v_profile_id) AND r.reporter_id != v_profile_id))
    ORDER BY r.created_at DESC;
END;
$$;

-- 3. Enhanced Sent Reports
...
BEGIN
    SELECT id, user_id INTO v_profile_id, v_user_id
    FROM public.profiles
    WHERE id = reporter_profile_id OR user_id = reporter_profile_id
    LIMIT 1;
...
GRANT EXECUTE ON FUNCTION public.get_user_related_reports TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_user_related_reports TO service_role;
