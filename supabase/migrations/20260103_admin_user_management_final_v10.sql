-- [FIX] Unified Admin Fixes V10: New Users Stat + Aggregate Stats + Sorting + Reports
-- 1. get_admin_users_list: Added global stats (admins, banned, online, reported, NEW USERS)
...
RETURNS TABLE (
  ...
  global_new_user_count bigint -- Added for New Users (Last 1 Week)
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_new_user_count FROM auth.users WHERE created_at >= now() - interval '7 days';
  ...
  SELECT 
    ...
    urc.r_count,
    v_admin_count, v_banned_count, v_online_count, v_reported_count, v_new_user_count
  FROM auth.users au
  JOIN public.profiles pp ON au.id = pp.user_id
