...
-- V3: Handles deleted content by checking content_snapshot
-- 1. Resolves IDs (Profile UUID & Auth ID) to handle referencing ambiguity.
-- 2. Finds reports on existing content via Joins.
-- 3. Finds reports on DELETED content via content_snapshot JSON matching.
...
BEGIN
    ...
    WHERE 
        ...
        -- 4. 삭제된 콘텐츠에 대한 신고 (스냅샷 기반 조회)
        -- 스냅샷의 user.username (Auth ID)이 대상 사용자의 Auth ID와 일치하는 경우
        (
            r.content_snapshot IS NOT NULL 
            AND (
                r.content_snapshot->'user'->>'username' = v_user_id_text
                OR
                r.content_snapshot->'user'->>'id' = v_user_id_text -- 혹시 id 필드에 AuthID가 들어갈 경우 대비
            )
        )
