-- ============================================================================
-- 20260519_extend_coupon_target_types_and_update_checkout_rpc.sql
-- 쿠폰 사용처(target_type) 제약조건 확장 및 통합 결제 RPC 업데이트 (Phase 3-B)
-- ============================================================================

-- 1. promotions.target_type CHECK 제약조건 안전하게 확장
ALTER TABLE public.promotions DROP CONSTRAINT IF EXISTS promotions_target_type_check;

ALTER TABLE public.promotions ADD CONSTRAINT promotions_target_type_check 
    CHECK (target_type IN ('all', 'new_user', 'subscriber', 'subscription', 'goods'));

-- 2. 통합 결제 처리 RPC 함수 재정의 (구독 결제 쿠폰 target_type 검증 보강)
