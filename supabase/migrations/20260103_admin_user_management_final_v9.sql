-- [FIX] Unified Admin Fixes V9: Aggregate Stats + Sorting + Reports
-- 1. get_admin_users_list: Added global stats (admins, banned, online, reported)
...
-- 1. Enhanced User List with Global Stats
...
RETURNS TABLE (
  ...
  -- Global Stats (constant for all rows in the result set)
...
BEGIN
  ...
  -- Calculate Global Aggregate Stats (independent of filters)
  ...
  SELECT COUNT(*) INTO v_online_count FROM public.profiles WHERE is_online = true;
  
  -- Reported User Count (users who have at least one received report)
  ...
  SELECT 
    ...
    urc.r_count,
    v_admin_count, v_banned_count, v_online_count, v_reported_count
  FROM auth.users au
  JOIN public.profiles pp ON au.id = pp.user_id
