...
function validateNicknameField(nickRaw: string, t: (key: string) => string): { error: string; lang: Lang | null } {
  ...
  if (/(.)\1\1/.test(u))
    return { error: t('validation.nickname_no_consecutive'), lang: null };
  ...
  if (!hasOnlyOneScript(u))
    return { error: t('validation.nickname_single_script'), lang: null };
  ...
  if (u.length < min || u.length > max)
    return { error: t('validation.nickname_length').replace('{min}', String(min)).replace('{max}', String(max)), lang };
  return { error: '', lang };
}
...
export default function SignUpStep2Form({
  ...
  const validate = (withDupHints = false): boolean => {
    ...
    // 공통 검증
    const { error: nickMsg, lang } = validateNicknameField(nickname, t);
    setNickLang(lang);
  ...
  const nickDupStatus = async (): Promise<'available' | 'taken' | 'error'> => {
    const { error: msg, lang } = validateNicknameField(nickname, t);
    setNickLang(lang);
    if (msg) {
      setErrors(prev => ({ ...prev, nickname: msg }));
      return 'error';
    }
    const srvErr = lang ? await serverNicknamePolicyError(nickname, lang) : null;
    if (srvErr) {
      setErrors(prev => ({ ...prev, nickname: srvErr }));
      return 'error';
    }
    try {
      // 1) 서버 정책 검사 (욕설/예약어 등)
      // This comment is now slightly out of place as policy check is done above.
  ...
  const handleNickCheck = async () => {
    ...
    if (res === 'taken') {
      ...
      setErrors(prev => ({ ...prev, nickname: t('signup.error_nickname_taken') }));
    } else if (res === 'available') {
      setNickCheckResult('available');
      setErrors(prev => ({ ...prev, nickname: undefined }));
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      if (nRes === 'error') {
        setErrors(prev => ({ ...prev, nickname: t('signup.error_nickname_check_retry') }));
        return;
      }
        ...
        <div>
          ...
          {nickname && (
            <p className="text-[11px] text-gray-500 mt-1 ml-3">
              ...
              <span className="font-medium">{langLabel(nickLang) || t('signup.nickname_lang_unknown')}</span>
              {nickLang ? ` · ${t('signup.nickname_hint', { min: LEN[nickLang][0], max: LEN[nickLang][1] })}` : ''}
            </p>
          )}
        </div>
