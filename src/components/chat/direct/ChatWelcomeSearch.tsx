  ...
  return (
    <div className="relative flex flex-col h-full overflow-hidden bg-secondary">
      ...
      */}
      ...
      >
        <div className={`relative w-full max-w-lg transition-all duration-500 ${!hasSearchContent ? 'scale-105' : 'scale-100'}`}>
          <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
            <Search className={`h-5 w-5 transition-colors ${hasSearchContent ? 'text-primary' : 'text-muted-foreground'}`} />
          </div>
          <input
            type="text"
          ...
          <div className="max-w-3xl mx-auto w-full py-4 space-y-6 pb-20">
            {isHistorySearching ? (
               ...
               <div className="flex flex-col items-center justify-center py-24 text-muted-foreground text-center">
                 ...
                 <p className="text-lg font-medium">{t('chat.no_result', '검색 결과가 없습니다.')}</p>
                 <p className="text-sm mt-1 opacity-70">{t('chat.no_search_result_desc', { query, defaultValue: `'${query}'에 대한 대화 내역을 찾을 수 없습니다.` })}</p>
               </div>
            ) : (
                ...
                {historySearchResults?.users?.length > 0 && (
                  <div className="space-y-3">
                    <div className="flex items-center px-3">
                      ...
                      <span className="font-semibold text-sm text-foreground">{t('chat.search_result_rooms', '채팅방')} ({historySearchResults.users.length})</span>
                    </div>
                        ...
                        return (
                          ...
                          >
                            {otherUser.avatar_url ? (
                              <img src={otherUser.avatar_url} alt={otherUser.nickname} className="w-12 h-12 rounded-full object-cover shadow-sm border border-border" />
                            ) : (
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-sm text-muted-foreground truncate">
                                {t('chat.last_message_label', '마지막 대화')}: {chat.last_message_at ? new Intl.DateTimeFormat(i18n.language, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }).format(new Date(chat.last_message_at)) : '-'}
                              </p>
                            </div>
                ...
                {historySearchResults?.messages?.length > 0 && (
                  <div className="space-y-3">
                    <div className="flex items-center px-3">
                      ...
                      <span className="font-semibold text-sm text-foreground">{t('chat.search_result_messages', '대화 내용')} ({historySearchResults.messages.length})</span>
                    </div>
                    <div className="bg-secondary border border-gray-300 dark:border-gray-600 rounded-xl overflow-hidden divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        >
                          <div className="flex items-start gap-4">
                             {msg.sender?.avatar_url ? (
                                <img src={msg.sender.avatar_url} alt={msg.sender.nickname} className="w-10 h-10 rounded-full object-cover mt-1 border border-border" />
                             ) : (
                             ...
                             <div className="flex-1 min-w-0">
                               <div className="flex justify-between items-center mb-1.5">
                                 ...
                                 <span className="text-xs text-muted-foreground/60 tabular-nums">
                                   {new Intl.DateTimeFormat(i18n.language, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }).format(new Date(msg.created_at))}
                                 </span>
                               </div>
                               <div className="text-[15px] leading-relaxed text-muted-foreground/80 line-clamp-2 break-all">
                                 <HighlightText text={msg.content} query={query} className="bg-yellow-100 dark:bg-yellow-900/40 text-yellow-800 dark:text-yellow-200 px-0.5 rounded" />
                               </div>
                             </div>
                          </div>
                        </div>
