...
export default function GoodsPage() {
  ...
  const {
    banners: subHeroBanners,
    trackClick,
    trackView,
  } = useMarketingBanners('hero_slide', 'goods');
  ...
  return (
    <>
      <Helmet>
        <title>{t('goods.seo.title')}</title>
        <meta name="description" content={t('goods.seo.description')} />
        <meta name="keywords" content={t('goods.seo.keywords')} />

        {/* Open Graph - 카톡/페북 등 공유 미리보기 */}
        <meta property="og:title" content={t('goods.seo.og_title')} />
        <meta property="og:description" content={t('goods.seo.og_description')} />
        <meta property="og:type" content="website" />
        <meta property="og:url" content="https://arakorean.com/goods" />

        {/* Twitter / X 공유 미리보기 */}
        <meta name="twitter:card" content="summary" />
        <meta name="twitter:title" content={t('goods.seo.og_title')} />
        <meta name="twitter:description" content={t('goods.seo.og_description')} />

        {/* 대표 URL */}
        <link rel="canonical" href="https://arakorean.com/goods" />
      </Helmet>
          ...
          <div className="bg-white dark:bg-card rounded-2xl shadow-xl dark:shadow-none dark:border border-gray-800 p-2 mb-8 flex wrap justify-center gap-2">
            {CATEGORIES.map(cat => (
              <button
                ...
                className={`px-6 py-3 rounded-xl text-sm font-semibold transition-all ${activeCategory === cat ? 'bg-primary text-white scale-105' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
              >
...
function ProductCard({ product, navigate, t }: { product: Product; navigate: any; t: any }) {
  ...
  const { translatedText: translatedName } = useAutoTranslation(
    product.name,
    `goods_name_${product.id}`,
    currentLang,
  );
  ...
  return (
    <motion.div
      ...
      className="bg-white dark:bg-card rounded-2xl overflow-hidden shadow-sm hover:shadow-xl dark:border border-gray-800 transition-all group cursor-pointer"
    >
      <div className="relative aspect-square overflow-hidden bg-gray-200 dark:bg-gray-800">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1 z-20 pointer-events-none">
          {product.badge_new && (
            <Badge className="bg-[#00bfa5] text-white uppercase border-none">
              {t('goods.badge.new')}
            </Badge>
          )}
          {product.badge_best && (
            <Badge className="bg-amber-500 text-white uppercase border-none">
              {t('goods.badge.best')}
            </Badge>
          )}
          ...
          {product.status === 'soldout' && (
            <Badge className="bg-zinc-800 text-white uppercase border-none">
              {t('goods.badge.soldout')}
            </Badge>
          )}
        </div>
      </div>
