...
export default function StudyListPage() {
  ...
  const isAdFree =
    userPlan === 'premium' || userPlan === 'basic' || (isAdmin && userPlan !== 'free');
  ...
  const [levelFilter, setLevelFilter] = useState<TDifficulty>(
    (searchParams.get('level') ?? '') as TDifficulty,
  );
  ...
  const {
    ...
    trackClick: trackBannerClick,
    trackView: trackBannerView,
  } = useMarketingBanners('inline_card');
  ...
  const { translatedTexts: trTitles, status: trTitlesStatus } = useBatchAutoTranslation(
    clips.map((c: any) => c.title),
    clips.map((c: any) => `study_title_${c.id}`),
    targetLang,
  );
  const { translatedTexts: trDescs, status: trDescsStatus } = useBatchAutoTranslation(
    clips.map((c: any) => c.short_description || ''),
    clips.map((c: any) => `study_desc_${c.id}`),
    targetLang,
  );
  const { translatedTexts: trEpisodes, status: trEpisodesStatus } = useBatchAutoTranslation(
    clips.map((c: any) => (c.video && c.video?.length > 0 ? c.video[0].episode : '') || ''),
    clips.map((c: any) => `study_episode_${c.id}`),
    targetLang,
  );
  ...
  useEffect(() => {
    ...
    const fetchClips = async () => {
      ...
      let query = (supabase.from('study') as any).select(
        needsVideoFilter ? '*, video!inner(*)' : '*, video(*)',
        { count: 'exact' },
      );
  ...
  return (
    <>
      <Helmet>
        <title>{t('study.seo.title')}</title>
        <meta name="description" content={t('study.seo.description')} />
        <meta name="keywords" content={t('study.seo.keywords')} />

        {/* Open Graph - 카톡/페북 등 공유 미리보기 */}
        <meta property="og:title" content={t('study.seo.og_title')} />
        <meta property="og:description" content={t('study.seo.og_description')} />
        <meta property="og:type" content="website" />
        <meta property="og:url" content="https://arakorean.com/study" />

        {/* Twitter / X 공유 미리보기 */}
        <meta name="twitter:card" content="summary" />
        <meta name="twitter:title" content={t('study.seo.og_title')} />
        <meta name="twitter:description" content={t('study.seo.og_description')} />

        {/* 대표 URL */}
        <link rel="canonical" href="https://arakorean.com/study" />
      </Helmet>
                  ...
                  {clips.length > 0 ? (
                    clips.reduce<React.ReactNode[]>((acc, study, index) => {
                      ...
                      acc.push(
                        <ContentCard
                          ...
                          duration={
                            v?.runtime_bucket ||
                            (v?.runtime
                              ? `${Math.max(1, Math.floor(v.runtime / 60))}${t('study.formats.minutes_label', { defaultValue: '분' })}`
                              : null)
                          }
                      ...
                      if (!isAdFree && (index + 1) % 3 === 0) {
                        ...
                        const bannerIndex = hasBanners
                          ? Math.floor(globalIndex / 5) % inlineBanners.length
                          : 0;
                              ...
                              <div className="absolute inset-0 p-3 sm:p-5 flex flex-col justify-end">
                                <div className="flex items-center gap-2 mb-1">
                                  <span className="px-1.5 py-0.5 rounded-md bg-white/20 backdrop-blur-md text-[8px] sm:text-[9px] font-black text-white tracking-widest border border-white/10 uppercase">
                                    {banner
                                      ? t('study.promotion.ad_badge')
                                      : t('study.promotion.promo_badge')}
                                  </span>
                                </div>
                                <BannerTitle
                                  banner={banner}
                                  fallback={t('study.promotion.premium_title')}
                                />
                              </div>
                            </div>
                            ...
                            <div className="flex-1 px-2 py-2 sm:px-3 sm:py-2 md:px-4 md:py-2">
                              <div className="grid min-h-20 flex flex-col justify-center">
                                <div className="flex items-center justify-between opacity-50">
                                  <span className="text-[10px] md:text-[11px] font-black text-gray-400 uppercase tracking-tighter">
                                    Premium Collection
                                  </span>
...
function BannerTitle({ banner, fallback }: { banner: any; fallback: string }) {
  ...
  const { translatedText, status } = useAutoTranslation(
    title,
    `banner_title_inline_${banner?.id}`,
    i18n.language,
  );
