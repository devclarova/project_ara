...
const ChatMessageItem = ({
  ...
  const isMyMessage =
    msg.sender_id === targetUId ||
    (msg.sender && (msg.sender.id === targetUId || msg.sender.user_id === user?.id));
  const sender =
    msg.sender ||
          ...
          <div className={`flex flex-col ${isMyMessage ? 'items-end' : 'items-start'} gap-1 w-full`}>
            ...
            {translated && (
              <div className="mt-1 p-2 rounded-xl text-xs bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-700 max-w-full">
                <p className="font-bold text-[10px] text-primary mb-1 uppercase tracking-tighter">
                  Translation
                </p>
                {translated}
              </div>
                ...
                {attachments.map((att: any, attIdx: number) => {
                  ...
                  return (
                    <div key={att.id || attIdx} className="overflow-hidden rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm text-zinc-900 transition-all">
                      ...
                      {!isImage && !isVideo && (
                        <div
                          onClick={e => { e.stopPropagation(); onDownload(fileUrl, fileName); }}
...
const AdminUserActivityModal: React.FC<AdminUserActivityModalProps> = ({ isOpen, onClose, user }) => {
      ...
      if (msg.attachments && msg.attachments.length > 0 && !isDeleted) {
        msg.attachments.forEach((att: any) => {
          ...
          if (type === 'video' || /\.(mp4|webm|ogg|mov)$/i.test(url)) {
            ...
            media.push({ url, messageId: msg.id, date: msg.created_at, senderId: msg.sender_id, senderName: sender?.nickname || 'Unknown', senderAvatarUrl: sender?.avatar_url, type: 'image' });
          }
        });
      }
    });
  ...
  const scrollToBottom = (smooth = true) => {
    if (chatBodyRef.current) chatBodyRef.current.scrollTo({ top: chatBodyRef.current.scrollHeight, behavior: smooth ? 'smooth' : 'auto' });
  };
  ...
  const fetchLikes = async (page: number, refresh = true) => {
    ...
    try {
      ...
      if (refresh) { setData(items); setAllLikedItems(totalLikes); }
  ...
  useEffect(() => {
    if (selectedChat && !chatLoading && chatMessages.length > 0) { setTimeout(() => scrollToBottom(false), 100); }
  }, [selectedChat?.id, chatLoading]);

  useEffect(() => {
    if (isOpen) {
      setActiveTab('tweets'); setDetailStack([]); setSelectedChat(null);
      if (user) { fetchCounts(); fetchData(); }
    }
  }, [isOpen, user]);

  useEffect(() => {
    setDetailStack([]); setSelectedChat(null); setItemReplies([]); setData([]);
  ...
  const fetchRepliesForTweet = async (tweetId: string) => {
    ...
    try {
      ...
      if (targetReplyId) {
        ...
        let attempts = 0;
        const interval = setInterval(() => { if (scrollTarget() || attempts > 10) clearInterval(interval); attempts++; }, 200);
      }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`${user?.nickname}님의 활동 내역`} className="max-w-4xl h-[85vh] p-0 overflow-hidden">
      <div className="flex h-full overflow-hidden">
        <div className="w-48 bg-zinc-50 dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-800 p-4 space-y-2">
          {['tweets', 'replies', 'likes', 'chats'].map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab as Tab)} className={`w-full flex items-center justify-between p-3 rounded-xl text-sm font-bold transition-all ${activeTab === tab ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800'}`}>
              <div className="flex items-center gap-2">{tab === 'tweets' ? <FileText size={18} /> : tab === 'replies' ? <MessageSquare size={18} /> : tab === 'likes' ? <Heart size={18} /> : <MessagesSquare size={18} />} {tab === 'tweets' ? '게시글' : tab === 'replies' ? '댓글' : tab === 'likes' ? '좋아요' : '채팅'}</div>
              <span className="text-[10px] opacity-70">{tab === 'tweets' ? counts?.tweets_count : tab === 'replies' ? counts?.replies_count : tab === 'likes' ? (counts?.tweet_likes_count || 0) + (counts?.reply_likes_count || 0) : counts?.chats_count || 0}</span>
            </button>
          ))}
        </div>

        <div className="flex-1 flex flex-col overflow-hidden bg-white dark:bg-zinc-950">
          {loading ? <div className="flex-1 flex items-center justify-center"><Loader2 className="animate-spin text-primary" /></div> : (
              ...
              {currentItem || selectedChat ? (
                <div className="flex flex-col h-full overflow-hidden">
                  <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-950 z-10">
                    <button onClick={() => { if (selectedChat) setSelectedChat(null); else setDetailStack(prev => prev.slice(0, -1)); }} className="flex items-center gap-1 text-xs font-bold text-primary hover:underline"><ChevronLeft size={16} /> 목록으로</button>
                    ...
                    {activeTab === 'chats' && selectedChat ? (
                      <div className="flex-1 flex flex-col min-h-0">
                        <div className="relative flex-1 flex flex-col min-h-0 overflow-hidden">
                          <div ref={chatBodyRef} className="overflow-y-auto flex-1 px-4 py-4 space-y-6" onScroll={handleChatScroll}>
                            {chatLoading ? <div className="flex items-center justify-center h-full"><Loader2 className="animate-spin text-primary" /></div> : chatMessages.length === 0 ? <div className="flex items-center justify-center h-full text-zinc-400 text-sm">대화 내용이 없습니다.</div> : (
                          ...
                          {showScrollDownBtn && <button onClick={() => scrollToBottom(true)} className="absolute bottom-6 right-6 z-10 w-10 h-10 rounded-full bg-primary text-white flex items-center justify-center shadow-lg hover:scale-110 active:scale-95 transition-all"><ArrowDown size={20} /></button>}
                        </div>
                      </div>
                    ) : currentItem ? (
                      <div className="space-y-6">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-4">
                  {data.length === 0 && <p className="text-center text-zinc-500 mt-10 text-sm">데이터가 없습니다.</p>}
