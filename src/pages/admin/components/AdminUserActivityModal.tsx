...
const ChatMessageItem = ({
  ...
  return (
    <div
      key={msg.id}
      className={`flex flex-col gap-1 w-full mb-4 ${isMyMessage ? 'items-end' : 'items-start'}`}
    >
      <div className={`flex gap-2 items-start ${isMyMessage ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        <div
          className={`flex flex-col max-w-[70%] min-w-0 ${isMyMessage ? 'items-end' : 'items-start'}`}
        >
          <div
            className={`flex items-center gap-2 mb-1 px-1 ${isMyMessage ? 'flex-row-reverse' : 'flex-row'}`}
          >
          ...
          <div
            className={`flex flex-col ${isMyMessage ? 'items-end' : 'items-start'} gap-1 w-full`}
          >
            ...
            {hasAttachments && !isDeleted && (
              <div
                className={`flex flex-wrap gap-1 ${hasContent ? 'mt-1' : ''} ${isMyMessage ? 'justify-end' : 'justify-start'}`}
              >
                {attachments.map((att: any, attIdx: number) => {
                  ...
                  return (
                    <div
                      key={att.id || attIdx}
                      className="overflow-hidden rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm text-zinc-900 transition-all"
                    >
                      {isImage && (
                        <img
                          src={fileUrl}
...
const AdminUserActivityModal: React.FC<AdminUserActivityModalProps> = ({
  isOpen,
  onClose,
  user,
}) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (activeTab === 'tweets') {
        query = supabase
          .from('tweets')
          .select('*, profiles:author_id(nickname, avatar_url, user_id)')
        ...
        query = supabase
          .from('tweet_replies')
          .select(
            '*, profiles:author_id(nickname, avatar_url, banned_until, user_id), tweets(profiles:author_id(nickname, user_id))',
          )
          .eq('author_id', user.profile_id)
          .order('created_at', { ascending: false });
        ...
        `,
          ...
          .or(`user1_id.eq.${user.profile_id},user2_id.eq.${user.profile_id}`)
  ...
  const fetchLikes = async (page: number, refresh = true) => {
    ...
    setLoading(true);
    try {
      const { items, allLikedItems: totalLikes } = await tweetService.getLikedItems(
        user.profile_id,
        page,
        refresh ? [] : allLikedItems,
      );
      if (refresh) {
        setData(items);
    ...
    try {
      ...
      if (messages.length > 0) {
        ...
        const { data: profiles } = await supabase
          ...
          .or(
            `id.in.(${senderIds.map(id => `"${id}"`).join(',')}),user_id.in.(${senderIds.map(id => `"${id}"`).join(',')})`,
          );
      ...
      if (targetReplyId) {
        const scrollTarget = () => {
          ...
          if (el) {
            ...
            setTimeout(
              () => el.classList.remove('bg-primary/5', 'ring-2', 'ring-primary/20'),
              3000,
            );
            return true;
          }
  ...
  const fetchSubReplies = async (parentId: string) => {
    ...
    try {
      const { data, error } = await supabase
        .from('tweet_replies')
        .select(
          '*, profiles:author_id(nickname, avatar_url, banned_until, user_id), parent:parent_reply_id(profiles:author_id(nickname, user_id))',
        )
  ...
  const ActivityListItem = ({
    ...
    if (
      ...
      (item.attachments && item.attachments.some((a: any) => (a.file_type || a.type) === 'video'))
    ) {
    ...
    return (
      ...
      >
        ...
        <img
          src={avatar}
          className="w-8 h-8 rounded-full flex-shrink-0 object-cover bg-zinc-100 border border-zinc-100 dark:border-zinc-800"
        />
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[9px] text-zinc-400">
              {format(new Date(item.created_at || item.timestamp), 'MM.dd HH:mm')}
            </span>
          </div>
          ...
          <p
            className={`text-sm font-medium line-clamp-2 ${item.deleted_at ? 'text-zinc-400' : 'text-zinc-700 dark:text-zinc-300'}`}
          >
          ...
          <div className="flex items-center gap-3 mt-2 text-[10px] text-zinc-400 opacity-60 group-hover:opacity-100 transition-opacity">
            ...
            {item.deleted_at && (
              <span className="text-red-500 font-bold ml-auto text-[8px]">삭제됨</span>
            )}
          </div>
        </div>
      </div>
            ...
            <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
              {currentItem || selectedChat ? (
                <div className="flex flex-col h-full overflow-hidden">
                  ...
                  <div
                    className={`flex-1 flex flex-col min-h-0 ${selectedChat ? 'overflow-hidden' : 'overflow-y-auto'} p-6 pt-2 pb-10`}
                  >
                      ...
                      <div className="space-y-6">
                        ...
                        {!currentItem?.deleted_at ? (
                          ...
                          <div className="p-4 bg-red-50 dark:bg-red-950/20 text-red-600 rounded-xl text-center font-bold text-sm mt-10">
                            이미 삭제된 콘텐츠입니다. (
                            {format(new Date(currentItem.deleted_at), 'yyyy-MM-dd HH:mm')})
                          </div>
                        )}
                      </div>
                    ) : null}
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-4">
                  {data.length === 0 && (
                    <p className="text-center text-zinc-500 mt-10">데이터가 없습니다.</p>
                  )}
                  {activeTab === 'tweets' &&
                    data.map(t => (
                      <ActivityListItem key={t.id} item={t} onClick={() => pushToStack(t)} />
                    ))}
                  {activeTab === 'replies' &&
                    data.map(r => (
                      <ActivityListItem key={r.id} item={r} onClick={() => pushToStack(r)} />
                    ))}
                  {activeTab === 'likes' && (
                    <>
                      {data.map((item, idx) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="flex -space-x-2">
                            <img
                              src={c.user1?.avatar_url || '/images/default-avatar.svg'}
                              className="w-8 h-8 rounded-full border-2 border-white dark:border-zinc-900 object-cover bg-zinc-100"
                            />
                            <img
                              src={c.user2?.avatar_url || '/images/default-avatar.svg'}
                              className="w-8 h-8 rounded-full border-2 border-white dark:border-zinc-900 object-cover bg-zinc-100"
                            />
                          </div>
                          <div>
                            ...
                            <p className="text-[10px] text-zinc-400">
                              마지막 대화:{' '}
                              {c.last_message_at
                                ? format(new Date(c.last_message_at), 'yyyy-MM-dd HH:mm')
                                : '-'}
                            </p>
                          </div>
                        </div>
